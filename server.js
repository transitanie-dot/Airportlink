...
const PARTNER_EDITABLE_STATUSES = ['draft', 'rejected'];
...
app.post('/api/partner/company', async (req, res) => {
  try {
    const user = await getUserFromRequest(req);
    if (!user) return res.status(401).json({ error: 'Not signed in' });
...
app.post('/api/partner/zones', async (req, res) => {
  try {
    const user = await getUserFromRequest(req);
    if (!user) return res.status(401).json({ error: 'Not signed in' });
...
app.post('/api/partner/driver', async (req, res) => {
  try {
    const user = await getUserFromRequest(req);
    if (!user) return res.status(401).json({ error: 'Not signed in' });
...
app.post('/api/partner/vehicle', async (req, res) => {
  try {
    const user = await getUserFromRequest(req);
    if (!user) return res.status(401).json({ error: 'Not signed in' });
...
app.post('/api/partner/document', async (req, res) => {
  try {
    const user = await getUserFromRequest(req);
    if (!user) return res.status(401).json({ error: 'Not signed in' });
...
app.post('/api/partner/submit', async (req, res) => {
  try {
    const user = await getUserFromRequest(req);
    if (!user) return res.status(401).json({ error: 'Not signed in' });
    ...
    if (!state.partner) return res.status(400).json({ error: 'Fill in your company details first.' });
    if (!state.zones.length) missing.push('at least one service zone');
    if (!state.drivers.length) missing.push('at least one chauffeur');
    if (!state.vehicles.length) missing.push('at least one vehicle');
    if (!state.partner.payout_iban) missing.push('your bank details');
    if (!req.body?.contract_accepted && !state.partner.contract_accepted_at) {
      missing.push('the partner contract');
    }

    const country = state.partner.country || 'PT';
    const need = state.requirements.filter((r) => r.mandatory && r.country === country);

    need.filter((r) => r.scope === 'company').forEach((r) => {
    ...
    state.drivers.forEach((driver) => {
      need.filter((r) => r.scope === 'driver').forEach((r) => {
        const has = state.documents.some((d) =>
          d.requirement_code === r.code && d.driver_id === driver.id);
        if (!has) missing.push(`${r.label} for ${driver.full_name}`);
      });
    });

    state.vehicles.forEach((vehicle) => {
      need.filter((r) => r.scope === 'vehicle').forEach((r) => {
        const has = state.documents.some((d) =>
          d.requirement_code === r.code && d.vehicle_id === vehicle.id);
        if (!has) missing.push(`${r.label} for ${vehicle.plate}`);
      });
    });

    if (missing.length) {
      return res.status(400).json({
        error: 'Your application is not complete yet.',
        missing
      });
    }
...
// Revisão pelo admin.
app.post('/api/admin/partner/review', async (req, res) => {
  try {
    ...
    if (!partner_id || !['approved', 'rejected', 'in_review', 'suspended'].includes(decision)) {
      return res.status(400).json({ error: 'Missing partner_id or invalid decision.' });
    }

    const { data, error } = await supabase.from('driver_partners').update({
      status: decision,
      rejection_reason: decision === 'rejected' ? (reason || null) : null,
      reviewed_at: new Date().toISOString(),
      reviewed_by: admin.id,
      updated_at: new Date().toISOString()
    }).eq('id', partner_id).select('id, email, legal_name, status').single();
